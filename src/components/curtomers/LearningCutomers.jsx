...
const LearningCustomers = () => {
  ...
  // Fetch customers when vendorId is set
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `https://driving.shellcode.cloud/api/getLearningLicenseCustomers/${vendorId}`
        );
        const data = await response.json();

        if (
          data.message === "Learning license customers retrieved successfully"
        ) {
